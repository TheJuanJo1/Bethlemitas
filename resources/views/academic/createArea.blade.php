...
@section('first_frame')
    <div class="p-6 bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-200/50">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h2 class="text-xl font-extrabold text-slate-800 tracking-tight">Crear Asignatura</h2>
        </div>

        <form action="{{ route('store.area') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label for="area" class="block text-sm font-bold text-slate-700 mb-1">Nombre de la Asignatura </label>
                ...
                @error('area')
                    <p class="mt-2 text-xs text-rose-500 font-medium">La asignatura ya existe o el campo está vacío.</p>
                @enderror
            </div>
...
@section('list_table')
    <div class="p-6 bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-200/50">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <h2 class="text-xl font-extrabold text-slate-800 tracking-tight">Asignaturas Registradas</h2>
            
            <form action="{{ route('create.area') }}" method="GET" class="flex items-center gap-2">
                <div class="relative">
                    <input type="search" name="search" placeholder="Buscar Asignatura..." value="{{ request('search') }}"
            ...
            <div style="max-height: 530px; overflow-y: auto;" class="scrollbar-thin scrollbar-thumb-slate-200">
                <table class="w-full text-left border-collapse">
                    <thead class="sticky top-0 bg-slate-50 border-b border-slate-200 z-10">
                        <tr>
                            <th class="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Nombre de la Asignatura</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="2" class="px-6 py-10 text-center text-slate-400 italic text-sm">No hay asignaturas registradas.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
@section('second_frame')
    <div class="hidden p-6 bg-white rounded-2xl border-2 border-indigo-100 shadow-2xl animate-in fade-in zoom-in duration-300" id="modal_edit_area">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h2 class="text-xl font-extrabold text-slate-800 tracking-tight">Editar Asignatura</h2>
        </div>
        ...
        <form action="{{ route('update.area') }}" method="POST" class="space-y-4">
            ...
            <div>
                <label for="area_edit" class="block text-sm font-bold text-slate-700 mb-1">Nombre de la Asignatura *</label>
                ...
                @error('area_edit')
                    <p class="mt-2 text-xs text-rose-500 font-medium">Esta asignatura ya se encuentra registrada.</p>
                @enderror
            </div>
