                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 border">
                                @forelse ($group->teachers as $teacher)
                                    {{ $teacher->name }} {{ $teacher->last_name }}@if(!$loop->last), @endif
                                @empty
                            ...
                            <td class="px-4 py-2 border">
                                <button type="button" class="text-blue-600 hover:text-blue-800 hover:underline edit_group"
                                    data-id="{{ $group->id }}" name-group="{{ $group->group }}">
                                    Editar
                                </button>
                                <form action="{{ route('destroy.group', $group->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
